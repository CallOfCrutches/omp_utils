...
namespace omp
{
  namespace details
  {
    ...
    struct enumerate_iterator
    {
      ...
      struct pointer
      {
        explicit pointer( value_type&& value ) noexcept
      ...
      explicit enumerate_iterator( wrapped_iterator&& iter, std::ptrdiff_t value ) noexcept(
        std::is_nothrow_move_constructible_v<wrapped_iterator> )
    ...
    struct enumerate_pointer_adapter
    {
    ...
    struct enumerate
    {
      ...
      iterator begin() noexcept
      {
        return iterator( std::begin( container_ ), start_ );
      }

      iterator end() noexcept
      {
        return iterator( std::end( container_ ), start_ );
      }
  ...
  {
    return details::enumerate( details::enumerate_pointer_adapter( arr, arr + size ), start );
  }
  ...
  {
    return details::enumerate( details::enumerate_pointer_adapter( begin, end ), start );
  }
}
