...
TEST( omp_zip, iterator_types )
{
  ...
  auto f_is_const_it = std::is_same_v<decltype( initial1 )::const_iterator,
                                      std::tuple_element_t<0, std::decay_t<decltype( result.iterators() )>>>;

  auto s_is_it = std::is_same_v<decltype( initial2 )::iterator,
...
TEST( omp_zip, begin_end )
{
...

  {
    ...
    auto f_beg_is_const_it = std::is_same_v<decltype( initial1 )::const_iterator,
                                            std::tuple_element_t<0, std::decay_t<decltype( zip_begin.iterators() )>>>;

    auto s_beg_is_it = std::is_same_v<decltype( initial2 )::iterator,
...

  {
    ...
    auto f_cbeg_is_const_it = std::is_same_v<decltype( initial1 )::const_iterator,
                                             std::tuple_element_t<0, std::decay_t<decltype( zip_cbegin.iterators() )>>>;

    auto s_cbeg_is_it = std::is_same_v<decltype( initial2 )::const_iterator,
                                       std::tuple_element_t<1, std::decay_t<decltype( zip_cbegin.iterators() )>>>;
