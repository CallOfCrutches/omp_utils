...
TEST(omp_tuple_map_tuple, one_tuple) {
  ...
  auto result =
      omp::tuple_map([](const auto &val) { return val + 1; }, initial);
...
TEST(omp_tuple_map_tuple, multiple_tuple) {
  ...
  ASSERT_EQ(std::get<0>(result),
            std::get<0>(expected1) + std::get<0>(expected2));
  ASSERT_EQ(std::get<1>(result),
            std::get<1>(expected1) + std::get<1>(expected2));
  ASSERT_EQ(std::get<2>(result),
            std::get<2>(expected1) + std::get<2>(expected2));
  ASSERT_EQ(std::get<3>(result),
            std::get<3>(expected1) + std::get<3>(expected2));
  ASSERT_EQ(std::get<4>(result),
            std::get<4>(expected1) + std::get<4>(expected2));
}
...
TEST(omp_tuple_map_tuple, multiple_tuple_same) {
  ...
  auto result = omp::tuple_map(
      [](auto &val1, auto &val2) { return val1 + val2; }, expected1, expected1);
...
TEST(omp_tuple_map_tuple, multi_tuple_all_ref) {
  ...
  auto result = omp::tuple_map(
      [](auto &val1, auto &&val2) -> auto & { return val1 += val2; }, expected1,
      std::move(expected2));
...
namespace {
struct DummyFunctor {
  char &&operator()(char &c) { return std::move(c); }
...
TEST(omp_tuple_map_tuple, type_depends_on_lambda_rvalue_res) {
  ...
  auto result = omp::tuple_map(
      [](auto &&v) -> auto{ return v; }, std::move(expected1));
...
TEST(omp_tuple_map_array, one_array) {
  ...
  auto result =
      omp::tuple_map([](const auto &val) { return val + 1; }, expected);
...
TEST(omp_tuple_map_array, multiple_arrays) {
  ...
  std::for_each(std::begin(expected1), std::end(expected1),
                [](auto &val) { val--; });
...
TEST(omp_tuple_map_different_types, tuple_pair_array) {
  ...
  auto result =
      omp::tuple_map([](auto &v1, auto &v2, auto &v3) { return v1 + v2 + v3; },
...
TEST(omp_tuple_reduce_pair, one_pair) {
  ...
  auto result = omp::tuple_reduce(
      [](auto accum, auto &val) { return accum * val; }, 1, expected);
...
TEST(omp_tuple_reduce_pair, multiple_pairs) {
  ...
  auto result = omp::tuple_reduce(
      [](auto accum, auto &val1, auto &val2) { return accum * val1 * val2; }, 1,
...
TEST(omp_tuple_reduce_different_types, tuple_array_pair) {
  ...
  auto expected2 = std::pair{
      3,
      4,
  };
  auto expected3 = std::tuple{
      5,
      6,
  };

  auto result = omp::tuple_reduce(
      [](auto accum, auto &val1, auto &val2, auto &val3) {
        return accum * val1 * val2 * val3;
      },
      1, std::as_const(expected1), std::as_const(expected2), expected3);
...
TEST(constexpr_context, omp_tuple_reduce) {
  std::array<int, omp::tuple_reduce(DummyFunctorAgain(), 1,
...
TEST(constexpr_context, omp_tuple_map) {
  std::array<int, std::get<2>(omp::tuple_map(DummyFunctorAgain(),
                                             std::make_tuple(1, 2, 3)))>
      t;
