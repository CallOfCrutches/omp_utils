...
constexpr bool support_get_one_(decltype(std::get<0>(std::declval<Tuple>()),
                                         int())) {
  return true;
}
template <typename> constexpr bool support_get_one_(...) { return false; }

template <typename... Tuples> constexpr bool support_get_() {
...
template <typename... Tuples> constexpr bool same_size_() {
  using details::tuple_size;

  using FirstTuple =
      std::remove_reference_t<decltype(first_type_<Tuples..., void>())>;
  return (... && (tuple_size<FirstTuple>::value ==
...
template <std::size_t Idx, typename Callable, typename Value,
          typename... Tuples>
...
template <std::size_t... Idxs, typename Callable, typename Value,
          typename... Tuples>
constexpr auto tuple_reduce_(std::index_sequence<Idxs...>, Callable &&call,
                             Value initial, Tuples &&...tups) {
  (reduce_one<Idxs>(std::forward<Callable>(call), initial,
                    std::forward<Tuples>(tups)...),
   ...);
...
template <
    typename Callable, typename... Tuples,
    std::enable_if_t<sizeof...(Tuples) && details::support_get_<Tuples...>() &&
                         details::same_size_<Tuples...>(),
                     int> = 0>
constexpr auto tuple_map(Callable &&call, Tuples &&...tups) {
  using FirstTuple =
      std::remove_reference_t<decltype(details::first_type_<Tuples...>())>;
  return details::tuple_map_(
      std::make_index_sequence<std::tuple_size_v<FirstTuple>>(),
      std::forward<Callable>(call), std::forward<Tuples>(tups)...);
}

template <
    typename Callable, typename Value, typename... Tuples,
    std::enable_if_t<sizeof...(Tuples) && details::support_get_<Tuples...>() &&
                         details::same_size_<Tuples...>(),
                     int> = 0>
constexpr auto tuple_reduce(Callable &&call, Value &&initial,
                            Tuples &&...tups) {
  using FirstTuple =
      std::remove_reference_t<decltype(details::first_type_<Tuples...>())>;
  return details::tuple_reduce_(
      std::make_index_sequence<std::tuple_size_v<FirstTuple>>(),
