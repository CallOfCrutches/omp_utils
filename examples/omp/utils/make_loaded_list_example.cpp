...
namespace
{
  ...
  struct ConcreteIntegerManager: public IIntegerManager
  {
    ...
    {
      auto it = std::find_if( std::cbegin( Elements ), std::cend( Elements ),
  ...
  {
    std::shared_ptr<IIntegerManager> mng = std::make_shared<ConcreteIntegerManager>();
