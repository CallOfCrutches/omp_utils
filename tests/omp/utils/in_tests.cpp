...
TEST(omp_in, enum_elements) {
  ASSERT_TRUE(omp::in(TestEnumClass::A, TestEnumClass::B, TestEnumClass::C,
                      TestEnumClass::A));
...
struct TestSimple {
  ...
  auto begin() const noexcept { return &variable; }

  auto end() const noexcept { return (&variable) + 1; }
};
} // namespace

TEST(omp_in, container_in_simple) { ASSERT_TRUE(omp::in(42, TestSimple())); }
...
struct TestWithFind {
  ...
  auto begin() const noexcept { return static_cast<int *>(nullptr); }

  auto end() const noexcept { return static_cast<int *>(nullptr); }

  auto find(const key_type &) const noexcept { return &variable; }
};
} // namespace
